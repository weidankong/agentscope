...
def _generate_call_tool_code(host_tool_url: str) -> str:
    ...
    return (
        ...
        "    _resp = _req.post(\n"
        f'        "{host_tool_url}/call/" + tool_name,\n'
...
async def main():
    ...
    # 4. Inject call_tool function into code_sandbox
    call_tool_code = _generate_call_tool_code(host_tool_url)
    await code_sandbox.run_ipython_cell(code=call_tool_code)
    ...
    try:
        ...
        def _format_tool_summaries(tools):
            if not tools:
            ...
            for schema in tools:
                ...
                parameter_names = [n for n in parameters if isinstance(n, str)]
                parameter_summary = ", ".join(parameter_names) if parameter_names else "none"
                description = str(func.get("description") or "").strip() or "No description provided."
                lines.append(f"- `{name}`: {description} Parameters: {parameter_summary}.")
            return "\n".join(lines)

        tool_summaries = _format_tool_summaries(list(tool_server._mcp_tools.values()))
        toolkit.register_tool_function(
            execute_python_code,
            func_description=f"""Execute Python in an isolated sandbox.

            Inside the sandbox, `call_tool(tool_name, **kwargs)` is available as a built-in for
        ...
        agent = ReActAgent(
            name="Friday",
            sys_prompt=f"""You have one primary tool: execute_python_code.

            Prefer one execute_python_code call per request when possible.
            Its tool description contains the current `call_tool(...)` guidance, sandbox
            tool registry, and capability limits.

            To surface results from `execute_python_code`, end the code with `print(...)`; the sandbox does not 
