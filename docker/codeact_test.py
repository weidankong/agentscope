...
async def main() -> None:
    ...
    try:
        config = SandboxConfig(
            ...
            tools=[Read(), Write()],    # local backend, read/write will happen locally
        )
        ...
        async with Sandbox(config) as sandbox:
            ...
            # Test builtin Write tool: write to /workspace/hello.txt
            ...
            # Test builtin Read tool: read from /workspace/hello.txt
            read_result = await sandbox.call_tool(
                "Read",
